...
Thunderdome.makeDataTable = function(markers,polygons,map,oms,infoWindow) {
		...
		Thunderdome.data.each(function(item) {
			...
			_.each(tableCols, function(tableCol) {
				...
				else if(_.indexOf(Thunderdome.tableConfig.get('dateFormatCols'), tableCol) > -1 && typeof item.get(tableCol) == 'string') {
					rowData.push( { v: new Date(Date.parse(item.get(tableCol)) ), f: item.get(tableCol)});
				}
				else {
